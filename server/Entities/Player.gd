...
func _physics_process(delta):
	...
	if move_direction != prev_move_direction:
		...
		# This is to not send unnecessary data but still make sure the player has the latest position
		if move_direction == Vector2.ZERO:
			SyncTimer.set_wait_time(SlowTick)
		else:
			SyncTimer.set_wait_time(FastTick)
			
		SyncTimer.start()	# Restarts the timer with new time
	prev_move_direction = move_direction


# Updates all peers with this players position on timeout
